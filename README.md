...
After Candidate B is approved, `npm run scientific:lifelike-loop` performs a second target-free hill climb over editable dimensional controls: slab perspective, lamella extrusion, contact occlusion, face contrast, highlights, rim light, and restrained surface texture. Public FigureLabs examples inform only general visual-quality traits; the loop does not copy, trace, or compare pixels against any reference image. SVG, TikZ, PNG, semantic JSON, approval evidence, and complete mutation lineage are emitted for review.
