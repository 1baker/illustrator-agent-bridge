...
Scientific Figure Studio v1 adds manuscript/proposal physical profiles, deterministic multi-panel composition, immutable supplied scientific media, hash-pinned local analysis adapters, SHA-bound approvals, PDF output, and publication/scientific QA without changing `ScientificImageRequest.v1`. Run `npm run scientific:project`; see [docs/scientific-figure-studio.md](docs/scientific-figure-studio.md).

Prompt-only publication candidates are available through `ScientificPromptFigureRequest.v1`, `scientific:prompt-plan`, `POST /v1/scientific/prompt-plan`, and MCP `plan_publication_figure_from_prompt`. The workflow accepts a plain prompt without a reference image, keeps provider output coordinate-free, searches a deterministic population of mutable vector programs across density, stroke hierarchy, palette, and depth treatment, emits physical-size SVG/PNG/editable TikZ, runs scene-measured QA, and stops at digest-bound human approval. Approved designs can be promoted separately into a content-addressed presentation-only registry; retrieval cannot import claims, citations, labels, values, or supplied media. See [docs/prompt-only-publication-figures.md](docs/prompt-only-publication-figures.md).

The shape-built scientific-object benchmark removes the surrounding card-and-arrow infographic and asks the harder first question: can one scientific object communicate through morphology alone? `npm run scientific:shape-benchmark` produces six deterministic semicrystalline polymer-film programs from paths, polygons, linework, fills, gradients, and clipping, then stops at human visual review. See [docs/shape-built-scientific-object-benchmark.md](docs/shape-built-scientific-object-benchmark.md).

Once a direction is selected, `npm run scientific:shape-loop` binds that exact program to a SHA-256 approval digest and runs a bounded autonomous hill-climb over explicit geometry and appearance parameters. Every accepted and rejected mutation retains lineage, and the resulting artwork still stops at human visual review.

After Candidate B is approved, `npm run scientific:lifelike-loop` performs a second target-free, deterministic beam search over editable dimensional controls: slab perspective, lamella extrusion, contact occlusion, face contrast, highlights, rim light, and restrained surface texture. Its fitness is computed from scene geometry and semantics—visibility, relationship coverage, framing, depth-cue families, clipping, stroke consistency, and light/shadow order—not proximity to hidden control targets. Mutations are bounded, relative, reversible, and include rejected and no-op proposals. The exact approved scene and SVG/TikZ/PNG evidence are SHA-bound with generator, renderer, font, mutation-policy, and runtime digests. Public FigureLabs examples inform only general visual-quality traits; the loop does not copy, trace, or compare pixels against any reference image. SVG, TikZ, PNG, semantic JSON, approval evidence, and complete mutation lineage are emitted for review.

The reusable refinement layer also separates immutable scientific semantics from mutable presentation state and provides a subject-neutral depiction grammar for shells, membranes, slabs, filaments, porous networks, particle populations, vessels, cutaways, and interfaces. Presentation mutations that alter the approved scientific component graph fail closed.
